...
impl FileIndexer {
    ...
    fn cache_path() -> Option<PathBuf> {
        dirs::data_local_dir().map(|d| d.join("ccar-copilot").join(CACHE_FILENAME))
    }
...
fn match_substring(entry: &FileEntry, keyword_lower: &str) -> Option<SearchHit> {
    ...
    if let Some(pos) = entry.name_lower.find(keyword_lower) {
        ...
        // 回退到路径匹配（分数较低）
        if entry.path_lower.contains(keyword_lower) {
            Some(SearchHit {
                name: entry.name.clone(),
                path: entry.path.clone(),
                size: entry.size,
...
fn save_cache_to_disk(entries: &Arc<RwLock<Vec<FileEntry>>>) {
    let path = match dirs::data_local_dir().map(|d| d.join("ccar-copilot").join(CACHE_FILENAME)) {
        Some(p) => p,
        None => return,
    };
...
mod tests {
    ...
    fn test_match_substring() {
        let entry = FileEntry {
            ...
            path_lower: "c:\\project\\readme.md".to_string(),
            size: 1024,
            modified_secs: 0,
            is_directory: false,
        };
    ...
    fn test_match_exact() {
        let entry = FileEntry {
            ...
            path_lower: "c:\\test.txt".to_string(),
            size: 100,
            modified_secs: 0,
            is_directory: false,
        };
    ...
    fn test_multi_keywords() {
        let entry = FileEntry {
            ...
            path_lower: "c:\\my_project_readme.md".to_string(),
            size: 100,
            modified_secs: 0,
            is_directory: false,
        };
