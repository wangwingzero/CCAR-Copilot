...
/// 对于轻微倾斜的文本，这种方法足够使用。
pub fn crop_text_region(image: &DynamicImage, points: &[[f32; 2]; 4]) -> Result<DynamicImage, OcrError> {
    // 计算边界框
...
pub fn crop_and_rotate_text_region(
    ...
    // 计算文本区域的宽度和高度
    let width = ((points[0][0] - points[1][0]).powi(2) + (points[0][1] - points[1][1]).powi(2)).sqrt();
    let height = ((points[0][0] - points[3][0]).powi(2) + (points[0][1] - points[3][1]).powi(2)).sqrt();
